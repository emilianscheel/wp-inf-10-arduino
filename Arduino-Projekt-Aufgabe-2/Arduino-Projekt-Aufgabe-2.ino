...
const int LED2_PIN = 3; // LED ist auf dem digitalen Pin 2 
...
void setup() {
 ...
 pinMode(LED2_PIN, OUTPUT); // Initialisiere den LED-Pin als Ausgang
...
void loop() {
 if (digitalRead(BUTTON_PIN)==HIGH) //überprüft ob der Taster gedrückt wird
    if (buttonState == 1) { // überprüft, ob der Taster aus ist
      ...
      digitalWrite(LED2_PIN, LOW); // LED2 ausschalten
    ...
    digitalWrite(LED2_PIN, HIGH); // LED2 einschalten
